...
const BookDetails = () => {
  ...
  const handleFavorite = async (e) => {
    try {
      ...
      const response = await axios.put("http://localhost:3000/api/v1/add-book-to-favorite",
       body,
       { headers }
      )
      alert(response.data.message)
  ...
  const deleteBook = async (e) => {
    const response = await axios.delete(`http://localhost:3000/api/v1/delete-book/${id}`,
      { headers }
    )
    alert(response.data.message)
    nevigate("/all-books")
